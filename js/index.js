...
//functions
  ...
  function sectionClonerFunction() {
    ...
    //cloneContainer.id = cloneContainer1;
  ...
  function sectionRemoverFunction() {
    //I need to remove the cloned section
    // remove ONLY the cloned section.
    //Note: cloned section is appended as a whole
   const oldData = mainSection.lastChild;
   if (mainSection.childElementCount > 4){   
      mainSection.removeChild(oldData)
    };
   // when there is no cloned section all other sections are being removed one by one. need to fix this
   // fixed by using an if statement to only run the function if there are more than 4 elements in the main section
  }
...
//main content
  ...
  //section buttons
    ...
    let sectionRemoverContainer = document.createElement('div');
    ...
    sectionClonerContainer.style.width = "40%";

    sectionRemoverContainer.style.display = "flex";
    sectionRemoverContainer.style.flexDirection = "column";
    sectionRemoverContainer.style.justifyContent = "center";
    sectionRemoverContainer.style.alignItems = "center";
    sectionRemoverContainer.style.color = "dodgerblue";
    sectionRemoverContainer.style.border = "2px double magenta";
    sectionRemoverContainer.style.height = "275px";
    sectionRemoverContainer.style.width = "40%"

    sectionButtonContainer.appendChild(sectionClonerContainer)
    sectionButtonContainer.appendChild(sectionRemoverContainer)
    mainSection.prepend(sectionButtonContainer);
      ...
      //section remover
        let sectionRemoverH4 = document.createElement('h4');
        let sectionRemoverH4Text = document.createTextNode('Section Remover');
        let sectionRemoverP = document.createElement('p');
        let sectionRemoverPText = document.createTextNode('This button will remove any cloned section.');
        let sectionRemoverButton = document.createElement('button');
        let sectionRemoverButtonText = document.createTextNode('Remove section.');
        

        sectionRemoverH4.style.fontSize = "32px";
        sectionRemoverH4.style.textDecoration = "underline";   

        sectionRemoverP.style.width = "35%";
        sectionRemoverP.style.textAlign = "center";
        sectionRemoverP.style.paddingTop = "15px";
        sectionRemoverP.style.marginTop = "10px";
        sectionRemoverP.style.paddingBottom = "15px";
        sectionRemoverP.style.fontSize = "17px";
        sectionRemoverP.style.lineHeight = "2";

        sectionRemoverButton.style.border = '1px double black';
        sectionRemoverButton.style.color ="darkblue";
        sectionRemoverButton.style.fontSize = '16px';
        sectionRemoverButton.style.backgroundColor = 'white';
        sectionRemoverButton.style.width = '170px';
        sectionRemoverButton.style.height = '35px';
        sectionRemoverButton.style.marginTop = '30px';
        sectionRemoverButton.style.cursor = "pointer";

        sectionRemoverButton.addEventListener('mouseenter', (event) => {
          event.target.style.backgroundColor = "black";
          event.target.style.color = "white";
          // setTimeout( () => {
          //   event.target.style.backgroundColor = "initial";
        ...
        sectionRemoverButton.addEventListener('mouseleave', (event) => {
          event.target.style.backgroundColor = "initial";
          event.target.style.color = "darkblue";
        })
        sectionRemoverButton.addEventListener('click', sectionRemoverFunction);

        sectionRemoverH4.appendChild(sectionRemoverH4Text);
        sectionRemoverP.appendChild(sectionRemoverPText);       
        sectionRemoverButton.prepend(sectionRemoverButtonText);

        sectionRemoverContainer.appendChild(sectionRemoverH4) 
        sectionRemoverContainer.appendChild(sectionRemoverP) 
        sectionRemoverContainer.appendChild(sectionRemoverButton) 
